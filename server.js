...
// 1. Health Check (Very important for Render)
app.get('/', (req, res) => res.send("Pearlpix Server is Running"));
...
app.post('/activate', async (req, res) => {
    ...
    try {
        ...
        if (data.dev && data.dev !== "" && data.dev !== dev) return res.status(400).json({ status: "error", message: "Locked to another device" });
...
// 4. Start Server (Binding to 0.0.0.0 is the FIX)
